...
* It ties data more closely to the function that operate on it,
  and protects it from accidental modification from outside function.
...
/* In the procedure oriented approach, large programs are divided into
  smaller programs known as functions.
* In POP, a program is written as a sequence of procedures or function.
  In POP, each procedure (function) contains a series of instructions
  for performing a specific task.
* During the program execution each procedure (function) can be called
  by the other procedures.
* To call a procedure (function), we have to write function name only.
  While we concentrate onto the development of functions, we give very
* little attention to the data that are 
* being used by various functions.
* In POP, the major emphasis is on procedure (function) and not on the
  data. 
* In a multi-function program, many important data items are placed as 
  global so that they may be accessed by all the functions.
...
* Examples of procedural oriented programming language are COBOL,
  FORTRAN, PASCAL, C programming language etc.
